...
qgis_dev_labelingGui::qgis_dev_labelingGui( QgsVectorLayer* layer, QgsMapCanvas* mapCanvas, QWidget* parent )
    : QWidget( parent )
    , mLayer( layer )			// 图层
    , mMapCanvas( mapCanvas )	// 地图控件
    , mCharDlg( 0 )				// 字符窗口
    , mQuadrantBtnGrp( 0 )
    , mDirectSymbBtnGrp( 0 )
    , mUpsidedownBtnGrp( 0 )
...
qgis_dev_labelingGui::~qgis_dev_labelingGui()
{

}

void qgis_dev_labelingGui::init()
{
    // 初始化图层标注配置
    ...
    switch( lyr.placement ) // 设置标注位置
    {
    case QgsPalLayerSettings::AroundPoint: // 点周围
        ...
        break;
    case QgsPalLayerSettings::OverPoint: // 在点上
...
void qgis_dev_labelingGui::apply()
{
    // 将配置写到图层中
    writeSettingsToLayer();
    mFontMissingLabel->setVisible( false );
    //qgis_dev::instance()->markDirty();
    if ( mMapCanvas ) { mMapCanvas->refresh(); }
}

void qgis_dev_labelingGui::writeSettingsToLayer()
{
    QgsPalLayerSettings settings = layerSettings();
    settings.writeToLayer( mLayer );
}

QgsPalLayerSettings qgis_dev_labelingGui::layerSettings()
{
    QgsPalLayerSettings lyr;
    lyr.enabled = chkEnableLabeling->isChecked();

    bool isExpression;
    lyr.fieldName = mFieldExpressionWidget->currentField( &isExpression ); // 设置标注字段
    lyr.isExpression = isExpression; // 是否启用表达式

    lyr.dist = 0; // 距离
    lyr.placementFlags = 0; // 指定标注显示的位置
    lyr.centroidWhole = mCentroidRadioWhole->isChecked(); // 居中显示
    lyr.centroidInside = mCentroidInsideCheckBox->isChecked(); // 内居中显示
...
void qgis_dev_labelingGui::setDataDefinedProperty( const QgsDataDefinedButton* ddBtn, QgsPalLayerSettings::DataDefinedProperties p, QgsPalLayerSettings& lyr )
{
...
void qgis_dev_labelingGui::changeTextColor( const QColor& color )
{
    Q_UNUSED( color );
    updatePreview();
}

void qgis_dev_labelingGui::collapseSample( bool collapse )
{
    if ( collapse )
    {
...
void qgis_dev_labelingGui::showEngineConfigDialog()
{
    // TODO:
}

void qgis_dev_labelingGui::changeBufferColor( const QColor& color )
{
    Q_UNUSED( color )
    updatePreview();
}

void qgis_dev_labelingGui::updateUi()
{
...
void qgis_dev_labelingGui::updatePreview()
{
    scrollPreview();
...
void qgis_dev_labelingGui::scrollPreview()
{
    scrollArea_mPreview->ensureVisible( 0, 0, 0, 0 );
}

void qgis_dev_labelingGui::updatePlacementWidgets()
{
...
void qgis_dev_labelingGui::updateSvgWidgets( const QString& svgPath )
{
...
void qgis_dev_labelingGui::on_mPreviewSizeSlider_valueChanged( int i )
{
    mPreviewSize = i;
    updatePreview();
}

void qgis_dev_labelingGui::on_mFontSizeSpinBox_valueChanged( double d )
{
    mRefFont.setPointSizeF( d );
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mFontCapitalsComboBox_currentIndexChanged( int index )
{
...
void qgis_dev_labelingGui::on_mFontFamilyCmbBx_currentFontChanged( const QFont& f )
{
    mRefFont.setFamily( f.family() );
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mFontStyleComboBox_currentIndexChanged( const QString & text )
{
    QgsFontUtils::updateFontViaStyle( mRefFont, text );
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mFontUnderlineBtn_toggled( bool ckd )
{
    mRefFont.setUnderline( ckd );
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mFontStrikethroughBtn_toggled( bool ckd )
{
    mRefFont.setStrikeOut( ckd );
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mFontWordSpacingSpinBox_valueChanged( double spacing )
{
    mRefFont.setWordSpacing( spacing );
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mFontLetterSpacingSpinBox_valueChanged( double spacing )
{
    mRefFont.setLetterSpacing( QFont::AbsoluteSpacing, spacing );
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mFontSizeUnitWidget_changed()
{
...
void qgis_dev_labelingGui::on_mFontMinPixelSpinBox_valueChanged( int px )
{
...
void qgis_dev_labelingGui::on_mFontMaxPixelSpinBox_valueChanged( int px )
{
    // ensure max font pixel size for map unit labels can't be lower than min
...
void qgis_dev_labelingGui::on_mBufferUnitWidget_changed()
{
    updateFont( mRefFont );
}

void qgis_dev_labelingGui::on_mCoordXDDBtn_dataDefinedActivated( bool active )
{
...
void qgis_dev_labelingGui::on_mCoordYDDBtn_dataDefinedActivated( bool active )
{
    if ( !active ) //no data defined alignment without data defined position
    {
        enableDataDefinedAlignment( false );
...
void qgis_dev_labelingGui::on_mShapeTypeCmbBx_currentIndexChanged( int index )
{
    // shape background
...
void qgis_dev_labelingGui::on_mShapeRotationCmbBx_currentIndexChanged( int index )
{
...
void qgis_dev_labelingGui::on_mShapeSVGParamsBtn_clicked()
{
...
void qgis_dev_labelingGui::on_mShapeSVGSelectorBtn_clicked()
{
...
void qgis_dev_labelingGui::on_mPreviewTextEdit_textChanged( const QString & text )
{
    lblFontPreview->setText( text );
    updatePreview();
}

void qgis_dev_labelingGui::on_mPreviewTextBtn_clicked()
{
    mPreviewTextEdit->setText( QString( "Lorem Ipsum" ) );
    updatePreview();
}

void qgis_dev_labelingGui::on_mPreviewBackgroundBtn_colorChanged( const QColor &color )
{
    setPreviewBackground( color );
}

void qgis_dev_labelingGui::on_mDirectSymbLeftToolBtn_clicked()
{
    bool gotChar = false;
...
void qgis_dev_labelingGui::on_mDirectSymbRightToolBtn_clicked()
{
    bool gotChar = false;
    QChar dirSymb = mCharDlg->selectCharacter( &gotChar, mRefFont, mFontDB.styleString( mRefFont ) );
...
void qgis_dev_labelingGui::blockInitSignals( bool block )
{
...
void qgis_dev_labelingGui::blockFontChangeSignals( bool blk )
{
...
void qgis_dev_labelingGui::setPreviewBackground( QColor color )
{
...
void qgis_dev_labelingGui::syncDefinedCheckboxFrame( QgsDataDefinedButton* ddBtn, QCheckBox* chkBx, QFrame* f )
{
...
void qgis_dev_labelingGui::populateFontCapitalsComboBox()
{
...
void qgis_dev_labelingGui::populateFontStyleComboBox()
{
...
void qgis_dev_labelingGui::populateDataDefinedButtons( QgsPalLayerSettings& s )
{
...
void qgis_dev_labelingGui::updateFont( QFont font )
{
...
void qgis_dev_labelingGui::enableDataDefinedAlignment( bool enable )
{
    mCoordAlignmentFrame->setEnabled( enable );
}

void qgis_dev_labelingGui::optionsStackedWidget_CurrentChanged( int indx )
{
...
void qgis_dev_labelingGui::showBackgroundRadius( bool show )
{
...
void qgis_dev_labelingGui::showBackgroundPenStyle( bool show )
{
...
void qgis_dev_labelingGui::on_mShapeSVGPathLineEdit_textChanged( const QString& text )
{
